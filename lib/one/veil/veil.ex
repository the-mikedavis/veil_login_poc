defmodule One.Veil do
  @moduledoc """
  Veil's main context
  """
  alias One.Repo
  alias One.Veil.{User, Request, Session}
  alias OneWeb.Veil.{Mailer, LoginEmail}
  ...
  defp new_session_url(conn, unique_id) do
    ...
    cur_path = OneWeb.Router.Helpers.session_path(conn, :create, unique_id)
    
    OneWeb.Router.Helpers.url(cur_uri) <> cur_path
  end
  ...
  def verify(%Session{} = session) do
    verify(OneWeb.Endpoint, session)
  end

  def verify(%Request{} = request) do
    verify(OneWeb.Endpoint, request)
  end
